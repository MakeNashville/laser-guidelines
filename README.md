# OMTech 60W CO2 Laser
## Detailed Maintenance Instructions

Complete step-by-step procedures for all maintenance tasks.

---

# Per-Use Tasks

---

## Cleaning the Focus Lens

**Time:** 2-3 minutes
**Frequency:** After every use

### Materials Needed:
- Lens cleaning solution (optical grade)
- Lint-free wipes or lens tissue
- Compressed air (optional)

### Procedure:

1. **Power off the laser** and wait for any movement to stop.

2. **Access the lens:**
   - The focus lens is located in the laser head/nozzle assembly
   - Unscrew or unclip the lens holder
   - Carefully remove the lens, noting which side faces up
     - Hold lens by edges only – **never** touch optical surfaces   

3. **Clean the lens:**
   - Use compressed air to remove loose dust first (optional)
   - Fold a lint-free wipe and apply 2-3 drops of lens cleaning solution
   - Place lens on clean surface or hold carefully
   - Wipe in gentle circular motion, starting from center
   - Use a fresh section of wipe for each pass
   - Repeat until lens is clean

4. **Inspect the lens:**
   - Hold up to light at an angle
   - Look for scratches, pitting, or cloudiness
   - Check for burn marks around edges
   - If damaged, set aside and use spare lens

5. **Reinstall:**
   - Ensure lens is completely dry
   - Note: Curved side typically faces UP (toward the beam)
   - Secure lens holder but don't overtighten

### Warning Signs (Replace Lens):
- Visible scratches in coating
- Burn marks or discoloration
- Cloudy or hazy appearance
- Chipped edges

---

## Cleaning the Mirrors

**Time:** 5 minutes
**Frequency:** After every use

### Mirror Locations:
- **Mirror 1:** Rear left corner, receives beam from tube
- **Mirror 2:** On left side of the gantry (moves with Y axis)
- **Mirror 3:** On laser head (moves with X axis)

### Procedure:

1. **Power off the laser.**

2. **For each mirror:**
   - Apply lens cleaning solution to lint-free wipe
   - Gently wipe mirror surface in circular motion
   - Do NOT press hard – mirrors are front-surface coated
   - Use fresh wipe section for each mirror

3. **Inspect each mirror:**
   - Look for burn spots, pitting, or coating damage
   - Check that mirror is secure in mount
   - Note any damage for repair log

### Important Notes:
- Never spray solution directly on mirrors
- Never touch mirror surface with fingers
- Don't adjust mirror mounts unless trained in alignment
- Mirrors are typically gold or silicon coated

---

## Checking the Exhaust System

**Time:** 1 minute
**Frequency:** Before and after every use

### Procedure:

1. **Turn on exhaust fan** (may be automatic or manual switch).

2. **Check airflow:**
   - Place hand near exhaust outlet
   - Air should be moving strongly
   - Listen for normal fan operation

3. **Visual inspection:**
   - Look into machine cabinet for haze or smoke
   - Check that duct connections are secure
   - Ensure no visible blockages in duct

4. **After cutting:**
   - Let exhaust run for 1-2 minutes after last cut
   - Verify any smoke has cleared

### Warning Signs:
- Weak airflow
- Unusual sounds from fan
- Visible smoke escaping from cabinet
- Burning smell in room

---

## Emptying the Debris Tray and Cleaning Honeycomb

**Time:** 3-5 minutes
**Frequency:** After every use

### Procedure:

1. **Remove debris tray:**
   - Slide out tray from beneath honeycomb bed
   - Dispose of material scraps appropriately
   - Wipe tray if needed

2. **Clean honeycomb surface:**
   - Use brush to sweep loose debris
   - Remove any stuck material carefully
   - Vacuum if available (shop vac recommended)

3. **Check honeycomb condition:**
   - Look for warping or damage
   - Check for areas with heavy buildup

4. **Replace debris tray.**

---

## Verifying Water Cooling System

**Time:** 1 minute
**Frequency:** Before and after every use

### Procedure:

1. **Check water level:**
   - Look at reservoir/chiller
   - Water should be at or above minimum line
   - Add DISTILLED water only if low

2. **Verify flow:**
   - Look for flow indicator (spinning wheel or bubbles)
   - Listen for pump operation
   - Feel return line – should be slightly warm after use

3. **Check water quality:**
   - Water should be clear
   - No cloudiness, green tint, or particles
   - No algae growth

### Warning: Never operate laser if:
- Water level is low
- Pump is not running
- Water appears contaminated

---

# Weekly Tasks

---

## Deep Clean Honeycomb Bed

**Time:** 30-60 minutes (including soak time)
**Frequency:** Weekly

### Procedure:

1. **Remove honeycomb from machine:**
   - Lift out carefully (may be heavy)
   - Note orientation for reinstallation

2. **Initial cleaning:**
   - Brush off loose debris
   - Use shop vac to remove particles from cells

3. **Soaking (if heavily soiled):**
   - Fill utility sink or tub with warm water
   - Add mild degreaser (e.g., Simple Green)
   - Submerge honeycomb for 30 minutes
   - Agitate periodically

4. **Rinse and dry:**
   - Rinse thoroughly with clean water
   - Stand on edge to drain
   - Air dry completely before reinstalling
   - Can use compressed air to speed drying

5. **Reinstall:**
   - Ensure completely dry
   - Place in machine in correct orientation
   - Verify level and stable

---

## Clean and Lubricate Guide Rails

**Time:** 15 minutes
**Frequency:** Every 2 weeks (per OMTech)

### Materials:
- **White lithium grease** (per OMTech recommendation)
- Disposable cloths or paper towels
- Lint-free cloths

### Procedure:

1. **Power off machine.**

2. **Clean rails first:**
   - Use disposable cloth to wipe X-axis and Y-axis guide rails
   - Remove ALL old grease and debris
   - Rails should be completely clean

3. **Apply white lithium grease:**
   - Apply a **small amount** to each guide rail
   - Smooth it out along the rail
   - ⚠️ **Do not over-lubricate!** A little goes a long way
   - Over-lubrication traps dust and accelerates wear

4. **Distribute grease:**
   - Move gantry using arrow keys to work in the grease
   - Cover full range of motion on X and Y axes

5. **Lubricate bed screws:**
   - Wipe the 4 bed screws clean
   - Apply small amount of white lithium grease
   - Move bed up and down to distribute

6. **Remove excess:**
   - Wipe away any excess grease
   - You should have only a light film remaining

---

## Check Belt Tension

**Time:** 5 minutes
**Frequency:** Weekly

### Procedure:

1. **Locate belts:**
   - X-axis belt runs left-right
   - Y-axis belt(s) run front-back

2. **Check tension:**
   - Press belt with finger at midpoint
   - Should deflect approximately 5mm (1/4 inch)
   - Belt should feel taut but not guitar-string tight

3. **Listen test:**
   - Move gantry manually
   - Listen for any unusual sounds
   - Check for smooth motion

4. **If adjustment needed:**
   - Note the issue in maintenance log
   - Belt tensioning may require experienced member
   - Don't over-tighten (causes wear)

---

## Clean Air Assist Nozzle

**Time:** 5 minutes
**Frequency:** Weekly

### Procedure:

1. **Remove nozzle** from laser head.

2. **Clean interior:**
   - Use compressed air to blow out debris
   - Use pipe cleaner for stuck material
   - Check for clogs in air passage

3. **Check air line:**
   - Trace line from nozzle back to pump
   - Check for kinks or damage
   - Verify connections are secure

4. **Test air flow:**
   - Reconnect nozzle
   - Turn on air assist
   - Feel for air at nozzle tip

---

# Monthly Tasks

---

## Check Mirror Alignment

**Time:** 15-30 minutes
**Frequency:** Monthly

### Materials:
- Masking tape or thermal paper
- Safety glasses

### Procedure:

1. **Prepare for test:**
   - Power on machine
   - Home all axes

2. **Test Mirror 2:**
   - Place tape on Mirror 2
   - Fire single low-power pulse (~10%)
   - Check burn mark is centered on mirror

3. **Test Mirror 3:**
   - Place tape on Mirror 3
   - Fire pulse at front-left of bed
   - Fire pulse at front-right of bed
   - Both marks should be in same position

4. **Test at lens:**
   - Remove lens
   - Place tape over lens opening
   - Fire pulse at all four corners of bed
   - Marks should all be centered

5. **If misaligned:**
   - Note which mirror needs adjustment
   - Contact experienced member for alignment
   - Do not attempt if untrained

---

# Quarterly Tasks

---

## Full Coolant Replacement

**Time:** 30-45 minutes
**Frequency:** Every 3 months

### Materials:
- 5+ gallons distilled water
- Algaecide
- Bucket for draining
- Funnel (optional)

### Procedure:

1. **Power off and unplug machine.**

2. **Drain system:**
   - Open drain valve or disconnect lowest point
   - Drain into bucket
   - Tilt machine if needed to fully drain
   - Drain any chiller reservoir

3. **Flush system:**
   - Refill with clean distilled water
   - Run pump for 2 minutes
   - Drain again
   - Repeat if water was very dirty

4. **Refill with fresh water:**
   - Use only distilled water
   - Fill to proper level
   - Add algaecide (1 capful per gallon)

5. **Prime and test:**
   - Power on cooling system
   - Run for 5 minutes
   - Check for leaks at all connections
   - Top off water level as needed

6. **Record in log:**
   - Date of replacement
   - Any issues noted

---

## Clean Exhaust Fan and Ducting

**Time:** 30 minutes
**Frequency:** Quarterly

### Procedure:

1. **Disconnect power** to exhaust system.

2. **Access fan:**
   - Remove fan guard or cover
   - Some fans may need removal from duct

3. **Clean fan blades:**
   - Use brush to remove buildup
   - Wipe blades with damp cloth
   - Check for balance issues

4. **Clean ducting:**
   - Disconnect duct sections
   - Use duct brush or compressed air
   - Remove all debris
   - Check for holes or damage

5. **Reassemble:**
   - Reconnect all sections securely
   - Verify no air gaps at joints
   - Test fan operation

---

## Check Laser Tube Contacts

**Time:** 10 minutes
**Frequency:** Quarterly

### Safety: High voltage – power off and unplug!

### Procedure:

1. **Ensure machine is completely powered off and unplugged.**

2. **Locate tube connections:**
   - High voltage cathode at one end
   - Low voltage anode at other end
   - Note: CO2 tubes contain high voltage even when off – wait 5 minutes

3. **Visual inspection:**
   - Check for corrosion or oxidation
   - Look for burn marks or arcing damage
   - Verify connections are secure

4. **Clean if needed:**
   - Use electrical contact cleaner
   - Ensure connections are tight
   - Don't force connections

5. **Document condition** in maintenance log.

---

# Recording Completed Maintenance

---

## Slack Reporting Protocol

After completing any scheduled maintenance (weekly, monthly, or quarterly):

1. **Post in #laser-maintenance channel**

2. **Include:**
   - Maintenance type completed (weekly/monthly/quarterly)
   - Any issues found
   - Any supplies running low

3. **Example post:**
```
Completed weekly maintenance
✅ All items completed
⚠️ Honeycomb showing wear in center
📦 Running low on lens wipes - please reorder
```

4. **Log in physical checklist** posted on the laser table. 


---

*Questions? Post in #laser-maintenance.*
